	...
	<div id="product_list">
		...
		<div id="printPreview" style="border:1px dashed grey;margin-bottom:10px;padding:10px;">
			...
			<div class="product" style="margin-bottom:20px;">
				...
				<div class="overview">
					<?php
 						$kk = mysqli_query($conn, "SELECT product.* FROM product INNER JOIN cuts ON product.cut_id=cuts.id && product.pallet_id='$pallet_id'") or die(mysqli_error($conn));
						...
						while($product = mysqli_fetch_array($kk)){
								...
								if($unit == 'PP'){									
									$xx = "SELECT * FROM `weights` WHERE product_id='$product_id'";
				...
				<div class="palletDetail-<?php echo $pallet_id;?>" <?php if($ykRow['pallet_tare'] == ''){ echo 'style="display:none;"'; } ?>>
					...
					<table border="0" style="border:1px dashed grey">
						...
						<?php
							...
							$xxx2="SELECT weights.* FROM `weights` INNER JOIN product ON weights.product_id=product.id WHERE product.pallet_id='$pallet_id'";
						
							$x = "SELECT * FROM `product` WHERE pallet_id='$pallet_id'"; // This was group by cut
							...
							if($unit == 'PP'){
								...
								$xx = "SELECT id FROM `weights` WHERE product_id='$product_id' LIMIT 1";
								$yy = mysqli_query($conn, $xx) or die(mysqli_error($conn));
							...
							$productsX = "SELECT * FROM `weights` WHERE product_id='$product_id'";
							$productsY = mysqli_query($conn, $productsX) or die(mysqli_error($conn));
							// $numWeightsOfProduct = mysqli_num_rows($productsY);
							//$numWeightsOfProduct = getTotesNumProductsForCutOnIntake($intake_id, $cut_id);
							
							$xk = "SELECT * FROM `weights` WHERE product_id='$product_id'";
							$yk = mysqli_query($conn, $xk);
							$ykRow = mysqli_fetch_array($yk);
							$numWeightsOfProduct = mysqli_num_rows($yk);
							...
							$weights = mysqli_fetch_array($productsY);
								...
								<tr>
									...
									<?php
										...
										$xR = "SELECT * FROM `weights` WHERE product_id='$product_id'";
								...
								<?php
									...
									$xWeights = "SELECT * FROM `weights` WHERE product_id = '$productID'";

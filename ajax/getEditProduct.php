<?php
	...
	$weight_id = mysqli_real_escape_string($conn, $_GET['weight_id']);
	
	$x1 = "SELECT * FROM `product` WHERE id = '$product_id'";
	$y1 = mysqli_query($conn, $x1);
	$productRow = mysqli_fetch_array($y1);
	
	
	$x2 = "SELECT * FROM `weights` WHERE id = '$weight_id'";
	...
	$x3 = "SELECT * FROM `pallet` WHERE id = '$pallet_id'";
...
<form method="POST" id="addPalletForm" action="script_updateProduct.php">
	...
	<div class="float">
		...
		<?php
			...
			$xxx = "SELECT * FROM `brands` where id=$brandid";
			$yyy = mysqli_query($conn, $xxx);

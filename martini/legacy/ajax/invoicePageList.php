<?php
    ...
    $intakePicksheetSearchQuery = "SELECT pickerSheets.id FROM `pickerSheets`
                        ...
                        JOIN `intake` ON intake.id = pallet.intake_id WHERE intake.id = ? || pallet.id = ? GROUP BY pickerSheets.id";

    $intakeQueryResult = prepareExecuteQuery($intakePicksheetSearchQuery,'ii',[$term,$term]);

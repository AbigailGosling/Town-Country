...
function renderPDF($saleID){
	...
	$statementDate = time();
	$fileName = 'Reservation_'.$saleID.'_'.$statementDate.'.pdf';
	$pathToFile = 'PDF';

	PDFRenderer::generatePDFfromWeb('viewReservation.php?id='.$saleID,$pathToFile,$fileName);

	$x = "SELECT `customer_id` FROM `reservation` WHERE id=?";
	...
	$subject = "Reservation ".$saleID." from Town and Country Meats";
	$htmlBody = "<html>Please find attached a reservation from Town and Country Meats Group for ".$customer['businessname']." No: ".$saleID.".</html>";

	$x = "UPDATE `reservation` SET `sent`=1 WHERE id=?";

	$y = prepareExecuteQuery($x,'i',[$saleID]);

	return SLabsEmailer::send_email($customer_id,"RESERVATION",$customer_emails,$subject,$htmlBody,$pathToFile,$fileName,$saleID);

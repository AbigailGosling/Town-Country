<?php
	...
	if($name != '' && strlen($name) > 1){
		...
		$cutX = "SELECT * FROM `supplier` WHERE `name` LIKE ? ORDER BY `name` ASC";
		$cutY = prepareExecuteQuery($cutX,'s',['%'.$name.'%']);
		
		while($row = mysqli_fetch_array($cutY)){
		?>
		<table width="100%">
			<tr><td align="center" class="pos">
				<a href="manageSuppliers.php?id=<?php echo $row['id']; ?>" class="intake" style="margin-top:0px;">

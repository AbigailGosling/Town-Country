...
<div class="container">
    <?php
        ...
        $USER_IDS = array();
        $usersWithPicksResult = prepareExecuteQuery("SELECT user_from_id FROM `pickerSheets` WHERE completed=1 GROUP BY user_from_id");
    ...
    <?php
        ...
        if($form_user_id != null){
            ...
            if($form_user_id == 0){
                ...
                $usersResult = prepareExecuteQuery("SELECT * FROM users WHERE id IN ($form_user_id)");
            }
            ...
            while($user = mysqli_fetch_array($usersResult)){
                ...
                $total_paid_user = 0;
                $total_sales = 0;
                $myCustomersResult = prepareExecuteQuery("SELECT id FROM `customers` WHERE default_salesman_id=?",'i',[$user_id]);

                $customer_ids = [];
                while($customer = mysqli_fetch_array($myCustomersResult)){ array_push($customer_ids, $customer['id']); }
                $custs = count($customer_ids);
                    ...
                    <table width="100%" border="0">
                        <tr>
                            ...
                            <?php
                                ...
                                if ($custs > 0) $picksheetsResult = prepareExecuteQuery("SELECT GROUP_CONCAT(id) as id,customer_id FROM `pickerSheets` WHERE completed=1 && customer_id IN ($customer_ids) $dateQueryPiece GROUP BY customer_id");
                                else $picksheetsResult = prepareExecuteQuery("SELECT GROUP_CONCAT(id) as id,customer_id FROM `pickerSheets` WHERE completed=1 && customer_id IN (-1) $dateQueryPiece GROUP BY customer_id");

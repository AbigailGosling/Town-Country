...
<main>
	<div id="intakelist">
		<h1 class="int">RESERVATION LIST</h1>
		...
		<a href="reservationList.php" class="resetBtn">Clear</a>
...
<div id="btm"></div>
	<script type="text/javascript">
		...
		$(document).ready(function(){
			...
			$('#instantSearch').keypress(function(){
				if (event.keyCode === 13)
				{
					...
					var request = $.ajax({
						type: "POST",
						url: "ajax/page-list/reservationList.php",
						data: {
							searchterm: val
						},
						dataType: "html"
		...
		function loadRows(){
			...
			xhttp.open("POST", "ajax/page-list/reservationList.php", true);
		...
		function loadSearchDate(month, year){
			...
			xhttp.open("POST", "/ajax/page-list/reservationListDate.php", true);
			xhttp.setRequestHeader("Content-type", "application/x-www-form-urlencoded");
			xhttp.setRequestHeader('X-CSRF-TOKEN', "<?php echo csrf_token();?>");

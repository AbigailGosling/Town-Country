...
use App\Models\Location;
	...
	$palletID = request()->input('palletID');
 	...
 	if($cut != '' && $species_id != 0){
		$cutsY = prepareExecuteQuery("SELECT * FROM `cuts` WHERE `name` LIKE ? AND `species_id`=? AND `disabled` = 0 order by `name` ASC",'si',['%'.$cut.'%',$species_id]);
	}elseif($cut != ''){
		$cutsY = prepareExecuteQuery("SELECT * FROM `cuts` WHERE `name` LIKE ? AND `disabled` = 0 order by `name` ASC",'s',['%'.$cut.'%']);
	}else{
		$cutsY = prepareExecuteQuery("SELECT * FROM `cuts` WHERE `species_id`=? AND `disabled` = 0 order by `name` ASC",'i',[$species_id]);
	}
		...
		while($cutsRow = mysqli_fetch_array($cutsY)){
			...
			exit();
			$productsX = "SELECT * FROM `product` WHERE cut_id = ? GROUP BY cut_id,nationality_id ORDER BY cooling_id ASC";
			$productsY = prepareExecuteQuery($xk,'i',[$cut_id]);
			...
			while($productsRow = mysqli_fetch_array($productsY)){
				...
				$productsX2 = "SELECT * , product.id productid FROM `product` INNER JOIN `pallet` ON product.pallet_id=pallet.id WHERE product.status='0' AND pallet.intake_id=? && product.cut_id = ? && product.nationality_id=?";
				$productsY22 = prepareExecuteQuery($productsX2,'iss',[$intake_id,$cut_id,$nationality_id]);
				$products2Count = mysqli_num_rows($productsY22);

...
use App\Models\Location;
	...
	$palletID = request()->input('palletID');
 	...
 	if($cut != '' && $species_id != 0){
		...
		$cutsY = prepareExecuteQuery("SELECT * FROM `cuts` WHERE `name` LIKE ? order by `name` ASC",'s',['%'.$cut.'%']);
	}else{
		$cutsY = prepareExecuteQuery("SELECT * FROM `cuts` WHERE species_id='$species_id' order by `name` ASC",'i',[$species_id]);
	}
		...
		while($cutsRow = mysqli_fetch_array($cutsY)){
			...
			while($productsRow = mysqli_fetch_array($productsY)){
				...
				$products2Count = mysqli_num_rows($products2Count);

...
<main>
	...
	<div class="overview">
        ...
        <div class="overview_block">
            <div>
                ...
                <?php echo ($intake['site_id']!= '')?Site::find($intake['site_id'])->name:"Unknown"; ?>
			</div>
		</div>
        <div class="overview_block">
			<div>
                <label>T&C Number</label>
                <?php echo $intake['internal_num']; ?>
			</div>
		</div>
        <div class="overview_block">
			<div>
                ...
                <?php
                    $healthmark = mysqli_fetch_array(prepareExecuteQuery("SELECT * FROM `health_mark` WHERE id = ".$intake['health_id']));
                    echo $healthmark['name'] ?? "";
                ?>
			</div>
		</div>
        <div class="overview_block">
			<div>
                ...
                <?php echo $intake['import_num']; ?>
			</div>
		</div>
        <div class="overview_block">
            <div>
                <label>No Open Product</label>
                <?php echo $intake['packaging_notes']; ?>
			</div>
		</div>
	...
	<table border="1" cellpadding="5" style="display:inline-block;">
		...
		<?php
			...
			$yPallets = prepareExecuteQuery("SELECT id FROM `pallet` WHERE intake_id='$intake_id'",);
			while($pallet = mysqli_fetch_array($yPallets)){ array_push($PALLET_IDS, $pallet['id']); }
			$PALLET_IDSstr = implode(',',array_fill(0,count($PALLET_IDS),'?'));

			$yProductsGrouped = prepareExecuteQuery("SELECT * FROM product WHERE pallet_id IN ($PALLET_IDSstr) GROUP BY cut_id",str_repeat('i',count($PALLET_IDS)),$PALLET_IDS);
			...
			while($productGrouped = mysqli_fetch_array($yProductsGrouped)){
				...
				$yProducts = prepareExecuteQuery("SELECT id FROM product WHERE pallet_id IN ($PALLET_IDSstr) && cut_id=?",str_repeat('i',count($thisVars)),$thisVars);

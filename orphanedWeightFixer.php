...
$orphans = array();
$pickedWeightsQ = mysqli_query($conn,"SELECT `id` from `weights` where `status_id` = 1 and product_id in ($product_ids)");
while ($weight = mysqli_fetch_assoc($pickedWeightsQ)) 
{
    $isOrphanCheckQ = mysqli_query($conn,"SELECT * from `palletsOut` where `weight_ids` LIKE '%".$weight['id']."%'");
...
{
    mysqli_query($conn,"UPDATE `weights` SET `status_id` = 0 WHERE `id` IN (".implode(",",$orphans).")");

	...
	<div id="menu_wrasp">
		<form method="POST">
			<div class="formElement flex">
				<div>
				<?php
					$resultsColumn3 = mysqli_query($conn, "SELECT * FROM `system_settings`");

					while($page = mysqli_fetch_array($resultsColumn3)){
                        if (isset($_POST[$page['key_name']]))
                        {
                            $sql = "UPDATE `system_settings` SET `key_value` = '".mysqli_real_escape_string($conn,$_POST[$page['key_name']])."' WHERE `key_name` = '".$page['key_name']."'";
                            mysqli_query($conn, $sql);
                            $page['key_value'] = $_POST[$page['key_name']];
                            $update_done = true;
                        }
					?>

...
<?php
	...
	$cutsX = "SELECT * FROM `cuts` WHERE name LIKE '%$cut%' AND species_id='$species_id'";
	$cutsY = mysqli_query($conn, $cutsX) or die(mysqli_error($conn));
	
	while($cutsRow = mysqli_fetch_array($cutsY)){
		$cut_id = $cutsRow['id'];
		
		$productsX = "SELECT * FROM `product` WHERE cut_id = '$cut_id' AND cooling_id='$temperatureID' ORDER BY range_from ASC";
